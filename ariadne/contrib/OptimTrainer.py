...
class OptimTrainer(Trainer):
    ...
    #                elif self.args.fp16 and _use_apex:
     #                   torch.nn.utils.clip_grad_norm_(amp.master_params(self.optimizer), self.args.max_grad_norm)
                    ...
                    if (
                        self.args.evaluation_strategy.value == EvaluationStrategy.STEPS.value
                        and self.global_step % self.args.eval_steps == 0
                    ):
                        metrics = self.evaluate()
                        if metrics['eval_' + eval_metric] > best_score:
                            # we have better model here, let's save it
                            logger.info('Save better model (previous score:' + str(best_score) + ', new score:' + str(metrics['eval_' + eval_metric]))
