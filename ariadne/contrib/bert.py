    ...
    def fit(self, documents: List[TrainingDocument], layer: str, feature: str, project_id, user_id: str):
        ...
        for document in documents:
            ...
            for sentence in self.iter_sentences(cas):
                ...
                covered_text = cas.get_covered_text(sentence)

                # only train using new documents, ignore the ones already seen
                if covered_text not in self.trained_docs:
                    sentences.append(covered_text)
                    targets.append(label)

        processor = Processor(labels=list(set(targets)), data={"texts": sentences, "labels": targets})
        train_dataset = (
            TwitterDataset(data_args, tokenizer=self.tokenizer, processor=processor, mode=Split.train)
        )
        ...
        # add documents to trained document set
        self.trained_docs += sentences
