...
import pandas as pd
from transformers import AutoTokenizer, AutoModelWithHeads, AdapterType, InputFeatures, \
    TrainingArguments, InputExample, DataProcessor
from typing import List, Optional
from .OptimTrainer_old import OptimTrainerAdapter
import torch
...
adapter_name = "twitter_opmin_de"
num_labels = 4
orig_dataset = "/home/tilman/Repositories/twitter-opinion-mining/data/annotation_mace.tsv"

class AdapterClassifier(Classifier):
    tokenizer = None
    def fit(self, documents: List[TrainingDocument], layer: str, feature: str, project_id, user_id: str):
        ...
        # convert incoming cas documents into training data
        ...
        processor = Processor(labels=list(set(targets)), data={"texts": sentences, "labels": targets})
        train_dataset = (
            TwitterDataset(data_args, tokenizer=tokenizer, processor=processor, mode=Split.train)
        )

        trainer = OptimTrainerAdapter(
            model=model,
            args=training_args,
            train_dataset=train_dataset
        )
        trainer.set_adapter_name(adapter_name)
        ...
        #featurized_sentences = convert_examples_to_features(sentences, tokenizer, max_length)
...
#        featurized_sentences = featurizer.featurize([s.get_covered_text() for s in self.iter_sentences(cas)])
        ...
        processor = Processor(labels=labels, data={"texts": sentences, "labels": [None for i in range(len(sentences))]})
        test_dataset = (
            TwitterDataset(data_args, tokenizer=self._get_tokenizer(), processor=processor, mode=Split.test)
        )
        trainer = OptimTrainerAdapter(
            model=model,
            args=training_args
